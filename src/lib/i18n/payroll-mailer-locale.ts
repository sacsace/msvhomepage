...
function mailSettings(locale: SiteLocale): PayrollMailerMailSettingsCopy {
  return pickLocale(locale, {
    ko: {
      title: "메일 환경 설정",
      lead: "SMTP 계정 정보를 입력하고 저장한 뒤, 테스트 메일로 연결을 확인하세요.",
      smtpHost: "SMTP Host",
      smtpPort: "SMTP Port",
      smtpUser: "SMTP User",
    ...
    en: {
      title: "Mail settings",
      lead: "Enter your SMTP credentials, save, then send a test email to verify connectivity.",
      smtpHost: "SMTP host",
      smtpPort: "SMTP port",
      smtpUser: "SMTP user",
      smtpPassword: "SMTP password",
    ...
    zh: {
      title: "邮件环境设置",
      lead: "填写 SMTP 账号并保存，然后发送测试邮件以确认连接。",
      smtpHost: "SMTP 主机",
      smtpPort: "SMTP 端口",
      smtpUser: "SMTP 用户",
...
export function payrollMailerPageCopy(locale: SiteLocale): PayrollMailerPageCopy {
  ...
  const inner = pickLocale(locale, {
    ko: {
      ...
      sendCompleteSmtp: "상단 메일 환경 설정을 완료하고 저장한 뒤 발송할 수 있습니다.",
      ...
      statusOk: "성공",
      statusFail: "실패",
      sendNeedSmtp: "메일 환경 설정을 먼저 완료하고 저장하세요.",
