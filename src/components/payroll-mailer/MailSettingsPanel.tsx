...
export function MailSettingsPanel({ settings, onChange, mail }: MailSettingsPanelProps) {
  ...
  return (
    <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      {envHint && <p className="mt-3 rounded bg-blue-50 p-3 text-sm text-blue-800">{envHint}</p>}
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2">
        ...
        <div className="md:col-span-2">
          ...
          <input
            ...
            placeholder='MS Ventures Payroll <no-reply@yourdomain.com>'
            className="w-full rounded border border-slate-300 px-3 py-2 text-sm"
          />
        </div>
      ...
      {saveMessage && <p className="mt-3 text-sm text-slate-700">{saveMessage}</p>}
      {testMessage && (
        <p
