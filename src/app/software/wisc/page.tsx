...
import { wiscSoftwarePageCopy } from "@/lib/i18n/wisc-software-page-locale";
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const c = wiscSoftwarePageCopy(locale);
...
export default async function SoftwareWiscPage() {
  const locale = await getRequestLocale();
  const c = wiscSoftwarePageCopy(locale);
    ...
    <>
      ...
      <StandardPageBody className="space-y-12 sm:space-y-14">
        ...
        <section className={cardSection}>
          ...
          <div className="mt-4 space-y-3">
            {overviewParagraphs.map((para, i) => (
              <p key={`wisc-overview-${i}`} className={bodyText}>
                {para}
              </p>
            ))}
          </div>
        ...
        {c.sections.map((s, si) => {
          ...
          return (
            <section key={`wisc-section-${si}`} className={cardSection}>
              <SectionTitle
                eyebrow={s.eyebrow}
                title={s.title}
                spacing="tight"
              ...
              <div className="mt-4 space-y-3">
                {paras.map((para, i) => (
                  <p key={`wisc-section-${si}-p-${i}`} className={bodyText}>
                    {para}
                  </p>
                ))}
                {s.bullets?.length ? (
